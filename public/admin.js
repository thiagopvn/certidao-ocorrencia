document.addEventListener("DOMContentLoaded", function () {
  ...
  // Estilos CSS para o status de envio de e-mail
  ...
  // Função para enviar email de notificação quando a certidão estiver pronta
  function sendCertificateEmail(
    ...
    // Mostrar carregamento ao enviar e-mail
    const emailStatusDiv = document.createElement("div");
    emailStatusDiv.className = "email-sending";
    emailStatusDiv.innerHTML =
      '<i class="fas fa-envelope"></i> Enviando notificação por e-mail...';

    // Adicionar ao modal
    const modalBody = document.getElementById("modal-body");
    if (modalBody) {
      modalBody.appendChild(emailStatusDiv);
    }
    ...
    // Enviar solicitação para a Cloud Function
    fetch(
      "https://us-central1-certidao-ocorrencia.cloudfunctions.net/sendCertidaoEmail",
      {
        method: "POST",
        headers: {
      ...
      .then((data) => {
        ...
        if (emailStatusDiv.parentNode) {
          emailStatusDiv.parentNode.removeChild(emailStatusDiv);
        }

        // Mostrar mensagem de sucesso
        showSuccessMessage(
          "E-mail enviado com sucesso!",
          "O solicitante foi notificado sobre a certidão."
        );
      })
      .catch((error) => {
        ...
        // Mostrar mensagem de erro, mas não impedir o fluxo
        showErrorMessage(
          "Aviso",
          "A certidão foi salva, mas ocorreu um erro ao enviar o e-mail de notificação."
        );
      });
  }
  ...
  if (updateStatusBtn) {
    updateStatusBtn.addEventListener("click", function () {
      ...
      // Buscar dados da ocorrência antes de atualizar (para ter o e-mail)
      database
        ...
        .then((snapshot) => {
          ...
          return database
            ...
            .then(() => {
              // Se o status mudou para Concluído e já tem uma certidão anexada, enviar e-mail
              if (
                newStatus === "Concluído" &&
                previousStatus !== "Concluído" &&
                ocorrenciaData.certidao &&
  ...
  if (uploadCertidaoBtn) {
    uploadCertidaoBtn.addEventListener("click", function () {
      ...
      storageRef
        ...
        .then(() => {
          // Buscar dados da ocorrência para obter o e-mail do solicitante
          return database.ref(`ocorrencias/${currentOccurrence}`).once("value");
        })
        .then((snapshot) => {
          if (snapshot.exists()) {
            const ocorrenciaData = snapshot.val();

            // Verificar se o status é Concluído e se temos o e-mail
            if (ocorrenciaData.status === "Concluído" && ocorrenciaData.email) {
              // Enviar e-mail de notificação
              sendCertificateEmail(
                currentOccurrence,
                ocorrenciaData.email,
