...
exports.enviarPesquisaSatisfacao = onSchedule({
  ...
  try {
    // Buscar ocorrências concluídas há 5 minutos (com margem de 1 minuto)
    const seisMinutosAtras = Date.now() - 6 * 60 * 1000;
    const quatroMinutosAtras = Date.now() - 4 * 60 * 1000;

    console.log(`Buscando ocorrências concluídas entre ${new Date(quatroMinutosAtras).toISOString()} e ${new Date(seisMinutosAtras).toISOString()}`);

    const snapshot = await getDatabase()
      .ref("/ocorrencias")
      .orderByChild("dataAtualizacao")
      .startAt(quatroMinutosAtras)
      .endAt(seisMinutosAtras)
      .once("value");
    ...
    snapshot.forEach((childSnapshot) => {
      ...
      if (
        ...
        ocorrenciasConcluidas.push({
          id: childSnapshot.key,  // Este é o ID da ocorrência no Firebase (occurrenceNumber)
          ...ocorrencia,
        });
      }
    });
